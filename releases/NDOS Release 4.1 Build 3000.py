...
NSET = ".fs/root/home/.ndos.elements/set.nset"
CALC = ".fs/root/home/.ndos.elements/calc.napp"
NSET_DATA = "{\n    \"settings-ver\": \"1.0.1\",\n    \"store-ver\": \"1.1.41\",\n    \"fs-ver\": \"2.0.1\",\n    \"techmode\": \"default\",\n    \"ndos-product-code\": \"NDOS Release 4.1 Build 3000\",\n    \"isModified\": \"False\"\n}"
CALC_DATA = "def calculator():    print('\nNDOS Calculator\n')    while True:        select = str(input'1 for basic, 2 for advanced mode> '))        if select == '1':            try:                num1 = float(input'Enter first number: '))                operator = str(input('Enter operation (+, -, *, /): ').strip())                num2 = float(input('Enter second number: '))                if operator == '+': result = num1 + num2                elif operator == '-': result = num1 - num2                elif operator == '*': result = num1 * num2                elif operator == '/':                    if num2 == 0: print('Error: Division by zero.'); continue                    result = num1 / num2                else: print('Invalid operator!'); continue                print(f'Result: {result}\n')            except ValueError: print('Error: Invalid numbers.\n'); continue            if input('Calculate again? (Y/N): ').strip().lower() != 'y': break        elif select == '2':            inp_math_op = input('Enter a full mathematical operation: ')            try:                adv_result = eval(inp_math_op)                print(f'Result is: {adv_result}')            except ValueError: print('Error: Invalid input.\n'); continue            if input('Calculate again? (Y/N): ').strip().lower() != 'y': break        else:            input('Enter a valid number!')"
current_path = ["home"]
...
def init_file_system():
    ...
    os.makedirs(os.path.join(FS_ROOT, "home", ".ndos.elements"), exist_ok=True)
    if not os.path.exists(NDOS_PKG):
        with open(NDOS_PKG, "w", encoding="utf-8") as pkgndos:
            pkgndos.write(f"{ndos_pkg_ver}")
    if not os.path.exists(NSET):
        with open(NSET, "w", encoding="utf-8") as nset:
            nset.write(NSET_DATA)
    if not os.path.exists(CALC):
        with open(CALC, "w", encoding="utf-8") as calc:
            calc.write(CALC_DATA)
...
def create_file(name):
    namename = list(name)
    namelen = int(len(name) - 1)
    nameextension = str(namename[(namelen-4)]+namename[(namelen-3)]+namename[(namelen-2)]+namename[(namelen-1)]+namename[(namelen)])
    if nameextension not in [".ndoc",".nset"]:
        name += ".ndoc"
    ...
    if os.path.exists(target_path):
        ...
        with open(target_path, "w", encoding="utf-8") as f:
            if nameextension == ".ndoc": f.write("=== NDOC ===\n---CONTENT---\n")
            elif nameextension == ".nset": f.write("=== NSET ===\n---CONTENT---\n")
...
def write_file(name):
    ...
    namelen = int(len(name) - 1)
    nameextension = str(namename[(namelen-4)]+namename[(namelen-3)]+namename[(namelen-2)]+namename[(namelen-1)]+namename[(namelen)])
    if namelen < 5 or nameextension not in [".ndoc",".nset"]:
        name += ".ndoc"
    ...
    if os.path.exists(target_path) and os.path.isfile(target_path):
        ...
        with open(target_path, "w", encoding="utf-8") as f:
            if nameextension == ".ndoc": f.write(f"=== NDOC ===\n")
            elif nameextension == ".nset": f.write(f"=== NSET ===")
            if nameextension == ".ndoc": f.write(f"AUTHOR: {current_username}\n")
            f.write(f"---CONTENT---\n")
            f.write(content)
...
def file_system_shell():
    ...
    while True:
        ...
        elif cmd == "touch" and args and len(args[0]) >= 5: create_file(args[0])
        elif cmd == "write" and args and len(args[0]) >= 5: write_file(args[0])
        elif cmd == "touch" and args and len(args[0]) < 5: print("File name must be at least 5 characters...")
        elif cmd == "write" and args and len(args[0]) < 5: print("Invalid file name. File names cannot be lower than 5 characters...")
